...
description: "Generates task specification documents from planning documents or conversation context. Creates task specs in tasks/task-specifications/ directory for the task-implementation module to process."
---
...
The specifications are consumed by the `task-implementation` module, which loads them via CLI and executes them using the `task-worker` skill.
