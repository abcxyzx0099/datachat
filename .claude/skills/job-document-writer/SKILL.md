...
Convert conversation context and user requirements into a structured job document, then save it to the `jobs/pending/` directory for pickup by the task monitoring system.
...
**BEFORE creating any job document**, verify that the task monitoring service is running. If the service is not running, the job document will never be processed and will sit idle in the `jobs/pending/` directory.
...
**Temp file pattern**: `jobs/pending/job-{description}.md.tmp`

**Example**: `jobs/pending/job-fix-auth-timeout.md.tmp`
...
| `JOBS_DIR` | `${PROJECT_ROOT}/jobs/pending` | Job pending directory |
...
```
jobs/pending/job-fix-auth-timeout.md.tmp
    ↓
jobs/pending/job-20260129-170500-fix-auth-timeout.md
```
...
FINAL_FILE=$(bash .claude/skills/job-document-writer/scripts/rename_job.sh jobs/pending/job-xxx.md.tmp)
```
...
# File: jobs/pending/job-fix-auth-timeout.md.tmp
# Content: (full job document)

# Step 4: Rename with generated timestamp
FINAL_FILE=$(bash .claude/skills/job-document-writer/scripts/rename_job.sh jobs/pending/job-fix-auth-timeout.md.tmp)
# Output: ✅ Job created: jobs/pending/job-20260129-170500-fix-auth-timeout.md
#         jobs/pending/job-20260129-170500-fix-auth-timeout.md
