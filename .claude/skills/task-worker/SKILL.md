...
flowchart LR
    ...
    ImplModule["task-management<br/>skill (uses task-queue CLI)"]
...
This skill is **automatically called** by the task-queue module when:
- A task specification has been created (by `task-specification-generation` skill)
- The module loads the task via `task-queue load`
- The daemon processes the task from the queue

**Note:** You typically don't invoke this skill directly. The task-queue module handles invocation.
