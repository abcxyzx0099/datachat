...
| Skill | Example |
|-------|---------|
| `/task-worker` | Executes task documents |
...
**How skills work:**
1. Invoked by name in prompt text: `/task-worker`
2. Claude Code CLI intercepts and routes to skill handler
3. Skill internally uses tools to complete its work

```
Prompt: "/task-worker Execute task at: task-xxx.md"
   ↓
CLI routes to /task-worker skill
   ↓
...
- Location: `{project_workspace}/tasks/task-documents/`
- Read and executed by the `/task-worker` skill
...
| **Tasks** | NO | File path passed to `/task-worker` |
...
- Tasks are executed by the `/task-worker` skill using those tools
