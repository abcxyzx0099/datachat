...
flowchart TD
    ...
    subgraph STAGE1[" "]
        ...
        subgraph P1["**Phase 1: Extraction & Preparation**"]
            ...
            S1["Step 1<br/>Extract .sav"]:::processingGreen
            S2["Step 2<br/>Transform"]:::processingGreen
            S3["Step 3<br/>Filter"]:::processingGreen
            filteredMeta[("filtered_metadata")]:::artifactStyle
        end

        subgraph P2["**Phase 2: New Dataset**"]
            S4["Step 4<br/>Gen Recoding"]:::processingBlue
            S5["Step 5<br/>Validate"]:::processingOrange
            S6["Step 6<br/>Review"]:::processingPurple
            S7["Step 7<br/>Gen Syntax"]:::processingGreen
            S8["Step 8<br/>Execute"]:::processingGreen
    ...
    subgraph STAGE2[" "]
        ...
        subgraph P3["**Phase 3: Indicators**"]
            S9["Step 9<br/>Gen Indicators"]:::processingBlue
            S10["Step 10<br/>Validate"]:::processingOrange
            S11["Step 11<br/>Review"]:::processingPurple
        end

        subgraph P4["**Phase 4: Tables**"]
            S12["Step 12<br/>Gen Tables"]:::processingBlue
            S13["Step 13<br/>Validate"]:::processingOrange
            S14["Step 14<br/>Review"]:::processingPurple
            S15["Step 15<br/>Gen Syntax"]:::processingGreen
            S16["Step 16<br/>Execute"]:::processingGreen
            crossTable[("cross_table")]:::artifactStyle
        end

        subgraph P5["**Phase 5: Statistics**"]
            S17["Step 17<br/>Gen Script"]:::processingGreen
            S18["Step 18<br/>Execute"]:::processingGreen
            statsSummary[("stats_summary")]:::artifactStyle
        end

        subgraph P6["**Phase 6: Filtering**"]
            S19["Step 19<br/>Gen Filter"]:::processingGreen
            S20["Step 20<br/>Apply"]:::processingGreen
            sigTables[("sig_tables")]:::artifactStyle
        end
    end
    ...
    subgraph STAGE3[" "]
        ...
        subgraph P7["**Phase 7: PowerPoint**"]
            S21["Step 21<br/>Gen PPT"]:::processingGreen
            ppt[("presentation")]:::artifactStyle
        end

        subgraph P8["**Phase 8: Dashboard**"]
            S22["Step 22<br/>Gen HTML"]:::processingGreen
            html[("dashboard")]:::artifactStyle
        end
    end

    %% Data flow
    sav1 --> S1 --> S2 --> S3 --> filteredMeta
    filteredMeta ==> S4 --> S5
    S5 -->|Valid| S6 -->|Approve| S7 --> S8 -.-> sav2 ==> newMetadata
    S5 -.->|Invalid| S4
    S6 -.->|Reject| S4

    newMetadata ==> S9 --> S10 -->|Valid| S11 -->|Approve| S12
    S10 -.->|Invalid| S9
    S11 -.->|Reject| S9

    S12 --> S13 -->|Valid| S14 -->|Approve| S15 --> S16 -.-> crossTable
    S13 -.->|Invalid| S12
    S14 -.->|Reject| S12

    crossTable ==> S17 --> S18 --> statsSummary ==> S19 --> S20 --> sigTables
    sav2 ==> S16
...
|-------|---------|----------|
| 🔵 **Blue** | AI-Orchestrated (LLM generates artifact) | Steps 4, 9, 12 |
| 🟢 **Green** | Deterministic (Python, PSPP, scipy) | Steps 1-3, 7-8, 15-22 |
| 🟠 **Orange** | Validation (Python checks) | Steps 5, 10, 13 |
| 🟣 **Purple** | Review (Human validates) | Steps 6, 11, 14 |
| 🟡 **Yellow** | Data Artifacts | `.sav`, `.csv`, `.json`, `.pptx`, `.html` |

| Line Style | Meaning |
|------------|---------|
| `-->` | Forward flow |
| `==>` | Major data flow between stages |
| `-.->` | Feedback loop |

### 2.2 Phase Descriptions

| Phase | Steps | Description | Input | Output |
|-------|-------|-------------|-------|--------|
| **1** | 1-3 | Extraction & Preparation | .sav file | Raw data, filtered metadata |
| **2** | 4-8 | New Dataset Generation | Filtered metadata, .sav | new_data.sav + new_metadata |
| **3** | 9-11 | Indicator Generation | new_metadata | Indicator definitions |
| **4** | 12-16 | Cross-Table Generation | new_metadata | Cross-table CSV + JSON |
| **5** | 17-18 | Statistical Analysis | Cross-table data | Statistical summary |
| **6** | 19-20 | Significant Tables Selection | Stats + tables | Significant tables only |
| **7** | 21 | Executive Summary | Significant tables | PowerPoint |
| **8** | 22 | Full Report | All tables | HTML dashboard |
...
| **[system-architecture.md](./system-architecture.md)** | System components and deployment |
