...
export function useFileUpload({
  ...
  const isDuplicate = (file: File, blocks: ContentBlock.Multimodal.Data[]) => {
    ...
    // Check for .sav files (SPSS files)
    if (file.type === "application/x-spss-sav" ||
        file.type === "application/x-spp" ||
        file.type === "application/octet-stream" && file.name.endsWith('.sav')) {
      return blocks.some(
        (b) =>
          b.type === "file" &&
          b.metadata?.filename === file.name,
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalidFiles.length > 0) {
      toast.error(
        "You have uploaded invalid file type. Please upload a JPEG, PNG, GIF, WEBP image, PDF, or SPSS .sav file.",
      );
    }
  ...
  const handlePaste = async (
    ...
    if (invalidFiles.length > 0) {
      toast.error(
        "You have pasted an invalid file type. Please paste a JPEG, PNG, GIF, WEBP image, PDF, or SPSS .sav file.",
      );
    }
    if (duplicateFiles.length > 0) {
      toast.error(
